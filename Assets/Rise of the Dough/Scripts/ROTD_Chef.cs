...
public class ROTD_Chef : MonoBehaviour 
{
    ...
    /// <summary>
    /// Internal cache of the local scale of the chef (direction)
    /// </summary>
	...
	{
        // set the local X scale to -1 if the direction is greater than zero, else keep the X scale at 1.
        _localScale = boneAnimation.mLocalTransform.localScale;
        _localScale.x = (direction.x > 0 ? -1f : 1f);
        boneAnimation.mLocalTransform.localScale = _localScale;
	}
